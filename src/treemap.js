...
const vis = {
  ...
  updateAsync: function (data, element, config, queryResponse, details, done) {
    ...
    const displayChart = function (root) {
    ...
    const createTreemap = function (data, vis, current_branch) {
      // Here's the tricky part -- we need to be able to recreate the visualization 
      // We now have access to the last Node we zoomed into but we still need to figure
      // out a way to recreate the d3 hierarchy so it renders properly.
    ...
    createTreemap(vis_data, vis, current_branch);
    done();
  }
};
